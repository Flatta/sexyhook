...
// 関数のテスト
SEXYHOOK_NOINLINE int add(int a, int b) {
	return a + b;
}
// クラスのテスト
class testclass {
...
public:
	...
	SEXYHOOK_NOINLINE bool checkCore(int a) {
		return a ? true : false;
	}
	// 2000年以上か？
	SEXYHOOK_NOINLINE bool isOver2000year() {
		return time(NULL) >= 946652400;	// 2000-01-01 00:00:00
	}
	SEXYHOOK_NOINLINE static void test() {
		{
			printf("\r\nクラスメソッドのフックその2\r\n");
			// これで書き換えてしまえばもう大丈夫w
		...
		// time() を埋め込んでいるプログラムも黙らせられるよ
		{
			...
			// テスト
			testclass2 cls;
			volatile bool r = cls.isOver2000year();
...
class Parent {
	int Dummy;
public:
	SEXYHOOK_NOINLINE virtual int f() {
		return 1;
	}
	SEXYHOOK_NOINLINE virtual int g() = 0;
};
class Child : public Parent {
	int Dummy2;
public:
	SEXYHOOK_NOINLINE int aaa() {
		return 0;
	}
	SEXYHOOK_NOINLINE virtual int f() {
		return 2;
	}
	SEXYHOOK_NOINLINE virtual int g() {
		return 3;
	}
};
// テンプレートクラスのテスト
...
public:
	SEXYHOOK_NOINLINE int add(_T a, _T b) {
		return a + b;
	}
};
// オペレータオーバーロードのテスト
class OpClass {
	int A;
public:
	SEXYHOOK_NOINLINE OpClass(int inA) : A(inA) {}
	SEXYHOOK_NOINLINE int getA() const {
		return this->A;
	}
	SEXYHOOK_NOINLINE OpClass operator+(const OpClass& x) {
		OpClass z(this->A + x.A);
		return z;
	}
};
// グローバルなオペレーターオーバーロードには、まだ未対応です。ごめんなさい。
SEXYHOOK_NOINLINE bool operator==(const OpClass& x, const OpClass& y) {
	return x.getA() == y.getA();
}
// オーバーロードのテスト
SEXYHOOK_NOINLINE int uadd(int a, int b) {
	return a + b;
}
SEXYHOOK_NOINLINE char uadd(char a, char b) {
	return a + b;
}
// テンプレート関数のテスト
// 掛け算
template<typename T> SEXYHOOK_NOINLINE T mul(const T& a, const T& b) {
	return a * b;
}
// おまけ dumpする関数
SEXYHOOK_NOINLINE static void hexDump(void *p, int size) {
	printf("\r\nDUMP %p:", p);
	int i = 0;
	for (; i < size; i++) {
		printf(" %02X", 0x000000FF & (unsigned int)*(((char *)p) + i));
		if (i % 16 == 15) {
			printf("\r\n");
		}
	}
...
class stdcallclass {
public:
	SEXYHOOK_NOINLINE int __attribute__((stdcall)) add(int a, int b) {
		return a + b;
	}
};
// fastcallのテスト
SEXYHOOK_NOINLINE int __attribute__((regparm(3))) fast_add(int a, int b) {
	return a + b;
}
// __fastcallなクラスメソッド.
class fastcallclass {
public:
	SEXYHOOK_NOINLINE int __attribute__((regparm(3))) add(int a, int b) {
		return a + b;
	}
};
#else
...
class stdcallclass {
public:
	SEXYHOOK_NOINLINE int __stdcall add(int a, int b) {
		return a + b;
	}
};
// fastcallのテスト
SEXYHOOK_NOINLINE int __fastcall fast_add(int a, int b) {
	return a + b;
}
// __fastcallなクラスメソッド.
class fastcallclass {
public:
	SEXYHOOK_NOINLINE int __fastcall add(int a, int b) {
		return a + b;
	}
};
#endif
...
public:
	int Secret;
	SEXYHOOK_NOINLINE classMethodCallTest() {
		this->Secret = 0;
	}
	SEXYHOOK_NOINLINE classMethodCallTest(int a) {
		this->Secret = a;
	}
	SEXYHOOK_NOINLINE int Add(int a) const {
		return a + this->Secret;
	}
	SEXYHOOK_NOINLINE int Sub(int a) const {
		return a - this->Secret;
	}
	SEXYHOOK_NOINLINE int setA(int a) {
		this->Secret = a;
	}
};
class constdoumei {
public:
	SEXYHOOK_NOINLINE int add(int a, int b) const {
		return a + b + 10;
	}
	SEXYHOOK_NOINLINE int add(int a, int b) {
		return a + b + 20;
...
private:
	SEXYHOOK_NOINLINE int private_add(int a, int b) {
		return a + b;
	}
public:
	SEXYHOOK_NOINLINE int public_add(int a, int b) {
...
int main() {
	// フック元呼び出しなどのテスト
	{
		...
		{
			volatile int r = c.Add(10);	// 10 + 2 = 12
		...
		// 別のメソッドの呼び出し
		{
			...
			SEXYHOOK_END();
			volatile int r = c.Add(10);	// 10 * (5 - 2) = 30
		...
		// 別のメンバ変数の呼び出し(ただし public だけ)
		{
			...
			SEXYHOOK_END();
			volatile int r = c.Add(10);	// 10 * 2 = 20
			SEXYHOOK_ASSERT(r == 20);
		}
		// 自分自身の再呼び出し
		{
	...
	// テンプレート関数のテスト
	{
		...
		SEXYHOOK_END();
		volatile int r1 = mul(9, 3);	// こっちのみ書き換える
		volatile long r2 = mul<long>(9, 3);
		printf("\r\nテンプレート関数のテスト\r\n");
	...
	// クラスメソッドのテスト
	{
		...
		SEXYHOOK_END();
		testclass myclass;
		volatile int r = myclass.add(10, 20);	// 足し算を引き算に上書きする.
		printf("\r\nクラスメソッドのテスト\r\n");
		printf("r1 : %d\r\n", r);
		SEXYHOOK_ASSERT(r == -10);
	}
	...
	// APIフックのテスト
	{
		...
		SEXYHOOK_END();
		// フックして
		volatile HANDLE h = HeapCreate(0, 0, 100);
		printf("\r\nAPIフックのテスト\r\n");
		printf("HeapCreate : %p\r\n", h);
	...
	// ダイナミックリンクのフック
	{
		...
		volatile int r = ((int(__stdcall *) (int, int))orignalProc)(1, 2);
	...
	// ダイナミックリンクの中でさらにダイナミックリンクのフック
	{
		...
		SEXYHOOK_ASSERT(orignalProc != NULL);
		volatile int r = ((int(__stdcall *) (int, int))orignalProc)(4, 2);	// 4 * 2 ではなく、 4 / 2を実行
	...
	// C関数strstrのフック
	{
		...
		const char *str2 = "hel";
		volatile const char *p2 = strstr(str1, str2);	// strstr("hello","hel") だと gcc が -O0なのに最適化するらしい.
		printf("\r\nC関数strstrのフック\r\n");
	...
	// gcc で動かない、、、、
	{
		...
		constdoumei c;
		volatile int r = c.add(10, 20);	// フック
		SEXYHOOK_ASSERT(r == -10);
	...
	// 今のところ VC6 + PlatformSDKでしか動作しない、、なんで？
	{
		...
		private_testclass c;
		// ここではまだフックしていない
		volatile int r = c.public_add(10, 20);
		SEXYHOOK_ASSERT(r == 30);
		// private メソッドのアドレスを算出してフック.
