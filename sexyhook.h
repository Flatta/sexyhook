...
// http://oshiete1.goo.ne.jp/qa1219076.html
...
#define SEXYHOOK_BEGIN(RET, CALLTYPE, FUNCADDRESS, ARGS) \
	...
	public: \
		...
		SEXYHOOK_REMACRO(CALLTYPE##__STATIC) RET CALLTYPE CallOriginalFunction ARGS { \
			volatile int mizumashi = 1; \
			for (volatile int i = 0 ; i < 1000 ; i++) { \
				mizumashi = mizumashi * i + 1; \
			} \
			throw 0; \
		} \
...
private:
	...
	uintptr_t CalcUkeFunctionAddress(void *inUkeFunctionProc, void *inVCallThisPointer) {
		...
		if ((uintptr_t)inUkeFunctionProc < 100) {
			...
			// http://d.hatena.ne.jp/Seasons/20090208/1234115944
	...
	int MakeTrampolineHookAsm(FUNCTIONHOOK_ASM *outBuffer, uintptr_t inUkeFunctionAddr, uintptr_t inSemeFunctionAddr) const {
		...
		if (inSemeFunctionAddr - inUkeFunctionAddr < 0x80000000) {// 2G以下のjmp
	...
	void OverraideFunction(void *inAddr, void *inNewSrc, int size) {
		...
		memcpy(inAddr, inNewSrc, size);
		// もとに戻したいんだけどどうすればいいの？
		// →面倒くさいっぽい http://stackoverflow.com/q/269314
#endif
	}
	// マシン語の長さを求めます。
	...
	static int OperandLength(const unsigned char *code, SEXYHOOK_CPU_ARCHITECTURE cputype) {
		...
		if (cputype == SEXYHOOK_CPU_ARCHITECTURE_X64) {// 64bit対応 その1
		...
		if (cputype == SEXYHOOK_CPU_ARCHITECTURE_X64) {// 64bit対応 その2
			if (registersize == 0x66 && *code == 0x2e) {// 66 2e 0f 1f 84 00 00 00 00 00  長いnot のように、 66 2e と入るときがあるらしい。
				code = code + 1;
				ex += 1;
			}
		...
		// 9b は謎w
		if (*code == 0x9b) {
			if (*(code + 1) == 0xdd) {// 9b dd b4 24 50 02 00 00  fsave  0x250(%rsp)
				return ex + 2 + 2 + 4;
			} else if (*(code + 1) == 0x97) {// 9b 97 66 00 ???
				return ex + 1 + 3;
			} else if (*(code + 1) == 0x3f) {// 9b 3f 08 ???
				return ex + 1 + 2;
			} else if (*(code + 1) == 0xc4) {// 9b c4 20   ???
				return ex + 1 + 2;
			} else {
				return ex + 1;
			}
		...
		if (len == 0) {
			...
			if (registersize == 0x66) {// 66 bf と来ると word 単位でアクセスするらしい
				return 3 + ex;
			} else {
				if (len == 0xe5 && (registersize64 >= 0x48 && registersize64 <= 0x4f)) {// 64bit拡張で 8バイトでアクセスできる命令
					return 9 + ex;
				}
				return 5 + ex;
			}
			...
			if (len == 0xfd) {// c7 85 00 fc ff ff 00 00 00 00
							  // mov         dword ptr [ebp-400h],0
				if (registersize == 0x66) {// 66 c7 と来ると word 単位でアクセスするらしい
					return 1 + 1 + modlen + 2 + ex;
				} else {
					return 1 + 1 + modlen + 4 + ex;
				}
			}
			if (len == 0xfc) {// c1 f8 0a
							  // sar	eax,0x0a
				return 1 + 1 + modlen + 1 + ex;
			}
			if (len == 0xfb) {// modrm によって長さが変わる
				if (reg == 0) {
